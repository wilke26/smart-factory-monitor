...
def test_loads_and_routes_one_artifact_per_machine(load: Mock, tmp_path: Path) -> None:
    ...
    registry = MachineModelRegistry.load(tmp_path, on_resolution=resolution)
    registry.evaluate(reading("press-02"))

    assert registry.machine_ids == ("press-01", "press-02")
    assert load.call_args_list == [
        call(tmp_path / "press-01.joblib", expected_machine_id="press-01"),
        call(tmp_path / "press-02.joblib", expected_machine_id="press-02"),
    ]
...
def test_rejects_empty_model_directory(tmp_path: Path) -> None:
    with pytest.raises(MlArtifactError, match="no ML artifacts"):
        MachineModelRegistry.load(tmp_path)


def test_rejects_missing_configured_machine_artifact(tmp_path: Path) -> None:
...
@patch("smart_factory.infrastructure.ml.registry.IsolationForestAnomalyDetector.load")
def test_does_not_activate_unconfigured_artifacts(load: Mock, tmp_path: Path) -> None:
    (tmp_path / "press-01.joblib").touch()
